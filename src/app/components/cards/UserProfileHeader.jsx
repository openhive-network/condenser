...
const UserProfileHeader = (props) => {
    ...
    const {
        name, location, about, website, cover_image, profile_image,
    } = profile
        ? profile.getIn(['metadata', 'profile']).toJS()
        : {};
    const [punkVerifiedId, setPunkVerifiedId] = useState(0);
    ...
    useEffect(() => {
        if (profile_image.indexOf('https://punks.usehive.com/api/avatar') === 0) {
            ...
            ssc.find('nft', 'PUNKinstances', { account: accountname, _id: parseInt(punkId) })
                .then((result) => {
                    setPunkVerifiedId(result?.length === 1 ? punkId : 0);
                })
                .catch((err) => {
                ...
                <div className="column" style={cover_image_style}>
                    ...
                    <h1>
                        <div className="UserProfile__Userpic">
                            <Userpic account={accountname} hideIfDefault punkId={punkVerifiedId} />
                        </div>
                        {name || accountname}
                        {' '}
                        <Tooltip
